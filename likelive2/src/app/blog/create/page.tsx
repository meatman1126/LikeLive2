...
export default function BlogCreatePage() {
  ...
  useEffect(() => {
    const loadArtists = async () => {
      try {
        const artists = await getMyArtistsAction();
  ...
  const handleSave = async (saveStatus: "DRAFT" | "PUBLISHED") => {
    ...
    startTransition(async () => {
      try {
        ...
        console.error("ブログの作成に失敗しました:", error);
        alert("ブログの作成に失敗しました");
      }
    });
  };
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <main className="mx-auto max-w-3xl px-6 py-8 md:py-12">
        ...
        {/* サムネイル: 未選択時はアイコンボタン */}
        {!selectedImage && !thumbnailPreview && (
          <button
            type="button"
        ...
        {/* サムネイル: クロップ済みプレビュー */}
        {thumbnailPreview && !selectedImage && (
      ...
      >
        <div className="flex h-full flex-col">
          ...
          <div className="flex items-center justify-between border-b border-gray-100 px-6 py-4">
            <h2 className="text-lg font-semibold text-gray-800">公開設定</h2>
            <button
              type="button"
              onClick={() => setIsSettingsPanelOpen(false)}
